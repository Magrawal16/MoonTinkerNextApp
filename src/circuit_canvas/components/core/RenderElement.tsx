...
import RgbLed from "@/circuit_canvas/components/elements/RgbLed";
...
export default function RenderElement(props: RenderElementProps) {
  ...
  return (
    ...
    >
      ...
      {props.showBody !== false && element.type === "rgbled" && (
        <RgbLed
          id={element.id}
          x={0}
          y={0}
          electrical={{
            red: {
              current: (element.computed as any)?.red?.current,
              forwardVoltage: (element.computed as any)?.red?.forwardVoltage,
              power: (element.computed as any)?.red?.power,
            },
            green: {
              current: (element.computed as any)?.green?.current,
              forwardVoltage: (element.computed as any)?.green?.forwardVoltage,
              power: (element.computed as any)?.green?.power,
            },
            blue: {
              current: (element.computed as any)?.blue?.current,
              forwardVoltage: (element.computed as any)?.blue?.forwardVoltage,
      ...
      {props.showBody !== false && element.type === "pushbutton" && (
        <PushButton
          id={element.id}
          x={0}
          y={0}
          draggable={false}
          selected={props.selectedElementId === element.id}
          pressed={element.properties?.pressed}
